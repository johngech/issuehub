...
export async function signUpAndAuthenticateBrowser(
	...
	if (!response.ok()) {
		...
		throw new Error(
			[
				`signUpAndAuthenticateBrowser: HTTP ${response.status()}`,
				body,
			].join("\n"),
		);
	}
...
/**
 * Navigate to a route and wait for the load to complete.
 *
 * Thin wrapper around page.goto that provides a single place to
 * tweak navigation behaviour across all tests.
 */
